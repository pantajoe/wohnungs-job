...
RSpec.describe UpdateGraylogJob, type: :job do
  ...
  describe Clockwork do
    ...
    it "runs the job once" do
      ...
      expect(Clockwork::Test.ran_job?("update_graylog")).to be_truthy
      expect(Clockwork::Test.times_run("update_graylog")).to eq 1
    end
    ...
    it "runs the job every minute over the course of an hour" do
      ...
      expect(Clockwork::Test.times_run("update_graylog")).to eq 6
    end
  end
end
